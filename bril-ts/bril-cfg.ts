...
function createBasicBlocks(prog: bril.Program) {
  ...
  let idx = 1
  let cur_block: BasicBlock = {name: null, instrs: [], idx: idx}
  for (let func of prog.functions) {
    if (func.name === "main") {
      ...
      if (cur_block.instrs.length > 0 || cur_block.name) {
        blocks.push(cur_block)
      }
    }
  }
...
function createCFG(blocks: BasicBlock[]) {
  ...
  let entryBlock:BasicBlock = {name: null, idx: 0, instrs:[{op:"nop", args:[], dest: "nop", type: "int"}]}
  let root:CFGNode = {name: entryLabel, block: entryBlock, next: []}
  nodeList.push(root)
  ...
  for (let node of nodeList) {
    ...
    if (node.block.instrs.length > 0 && successors.length == 0) {
      let lastInstr = node.block.instrs[node.block.instrs.length-1]
